...
public class ConfigurationAppService {
    ...
    public void deleteConfiguration(String code, String modifyBy) {
        ...
        long swBaselineRefCount = softwareBaselineRepository.countByAnchor(AnchorType.CONFIGURATION, code);
        if (swBaselineRefCount > 0) {
            throw new MdmBaseException(MdmErrorCode.HAS_CHILDREN_REFERENCE,
                    String.format("配置 %s 被软件基线引用，删除被拒绝（引用数量: %d）", code, swBaselineRefCount));
        }
