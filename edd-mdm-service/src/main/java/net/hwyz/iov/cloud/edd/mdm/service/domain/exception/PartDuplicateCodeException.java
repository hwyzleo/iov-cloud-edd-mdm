...
@Getter
public class PartDuplicateCodeException extends RuntimeException {
    ...
    public PartDuplicateCodeException(String conflictCode, String existingStatus) {
        super(String.format("零件 code 已存在: %s（已占用记录状态: %s）", conflictCode, existingStatus));
        this.conflictCode = conflictCode;
        this.existingStatus = existingStatus;
    }

    public PartDuplicateCodeException(String conflictCode, String existingStatus, Throwable cause) {
        super(String.format("零件 code 已存在: %s（已占用记录状态: %s）", conflictCode, existingStatus), cause);
        this.conflictCode = conflictCode;
        this.existingStatus = existingStatus;
    }
