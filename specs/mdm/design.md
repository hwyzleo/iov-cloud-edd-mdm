...
> **覆盖范围声明**：本 design 当前覆盖 **Product MDM 子域**（CR-001 ~ CR-005、CR-010 Option Family category 字段扩展）、**Party MDM 子域**（CR-006）、**EEAD 子域**（CR-007，含 US-039 ~ US-046 车载节点字典首版能力）、**Org 子域**（CR-008，含 US-047 ~ US-055 工厂 Plant 首版能力）与 **Material 子域**（CR-009，含 US-056 ~ US-070 物料品类 MaterialCategory + 零件 Part 首版能力；**CR-023，含 US-073 ~ US-082 Part 零件编码增强：系统化发号 + 字母代次 + 双键 + 发号来源治理**）。
>
...
>
> **Part 零件编码增强硬约束（CR-023，对 CR-009 Part 实体的增强，不新建实体、不引入新子域，命名空间完全沿用 Material）**：
> - 零件号 `code` 由系统统一发号，形态 = `[可选前缀] + N 位零填充流水 + 2 位字母代次`（软件件 `S` 前缀、硬件无前缀；N 位宽锁定为 8，对齐存量习惯如 `18100020AA` / `S1810021AA`）；**重定义** CR-009 US-059 的人工录入 code 为系统发号（与 CR-005 Configuration code 同范式）
> - 流水使用独立序列表 `mdm_material_part_seq` 的**全局单一计数器**（next_seq 单行、行锁自增，**不按 category / 前缀分段计数**），对齐 CR-005 `mdm_configuration_seq`；DRAFT 物理删除不回收序号；UK 兜底重试一次仍冲突返回 812917
> - 双键模型：surrogate `id`（BIGINT 技术主键，无语义、不可变、系统生成）承担内部 FK / 关系引用；`code` 仅作业务自然键 + 对外展示；内部关系与未来 BOM / SubstituteRelation 一律引用 `id`
> - 代次：末尾 2 位字母 `AA→AB→…→ZZ`；互换性变更升代次（新 code = 新记录、新 id）；小修订仅升 `drawing_version`，code / 代次不变；代次溢出返回 812918
> - 发号来源治理：`numbering_source` 枚举（MDM_GEN / PLM / IMPORT，默认 MDM_GEN），靠 `numbering_source` + `source_id` 区分与对账，不在 `code` 加专属号段；本期 PLM 未上线由 MDM 兜底发号，PLM 实际接入留待独立 CR（requirements OS-33）
> - 后台录入三层：默认系统取号（不暴露 code 录入框）/ 导入带号（numbering_source=IMPORT）/ 受 `mdm:material:part:code:manual` 权限手动录入兜底
> - 上游 code 冲突兜底：沿用 CR-005 US-030 两层决策范式（幂等更新保持原 code / 未命中按 code 是否被占用决定直采或本地兜底生成）
...
**CR-023 新增类（Part 零件编码增强，沿用现有扁平包结构，不引入子包）**：

| 类全限定名 | 说明 |
|---|---|
| **领域层（domain）** | |
| `service.domain.model.valueobject.NumberingSource` | 发号来源枚举（MDM_GEN / PLM / IMPORT，默认 MDM_GEN） |
| `service.domain.model.valueobject.PartGeneration` | 代次值对象（2 位字母，`AA→AB→…→ZZ`，封装 next() 递增与 overflow 判定、字母↔序号互转） |
| `service.domain.model.valueobject.PartCode` | 零件号值对象（封装 `[前缀] + N 位流水 + 2 位代次` 的拼装 / 解析 / 反解 base_no 与代次） |
| `service.domain.service.PartNumberingDomainService` | 发号领域服务（全局单一序列取号 → 前缀判定 → 拼接 code/base_no → UK 兜底重试一次；代次升级 next 代次生成；小修订仅升 drawing_version 不改 code；上游 code 冲突两层决策兜底） |
| `service.domain.repository.PartSeqRepository` | 零件号序列仓储接口（行锁自增 next_seq、单行全局计数器） |
| `service.domain.exception.PartCodeGenConflictException` | 812917（系统发号撞 UK，重试仍冲突） |
| `service.domain.exception.PartGenerationOverflowException` | 812918（代次溢出 ZZ） |
| `service.domain.exception.PartSeqOverflowException` | 812919（流水序号溢出 N 位上限） |
| `service.domain.exception.PartManualCodeForbiddenException` | 812920（无权限手动指定 code） |
| `service.domain.exception.PartNumberingSourceInvalidException` | 812922（发号来源为空或非法） |
| **应用层（application）** | |
| `service.application.dto.cmd.PartGenerationUpgradeCmd` | 代次升级命令（互换性变更，基于 base_no 生成新代次新记录） |
| `service.application.dto.cmd.PartMinorRevisionCmd` | 小修订命令（仅升 drawing_version，code 不变） |
| `service.application.dto.cmd.PartImportCmd` | 存量导入命令（numbering_source=IMPORT，带历史 code） |
| **基础设施层（infrastructure）** | |
| `service.infrastructure.persistence.po.PartSeqPo` | 序列表持久化对象（对应 mdm_material_part_seq） |
| `service.infrastructure.persistence.mapper.PartSeqMapper` | MyBatis Mapper（`UPDATE … SET next_seq = next_seq + 1`） |
| `service.infrastructure.persistence.repository.PartSeqRepositoryImpl` | 序列仓储实现（行锁自增，对齐 ConfigurationSeqRepositoryImpl） |

> **CR-023 复用既有类（不重建）**：`Part` 聚合根（新增 base_no / numberingSource / generation 字段与升代次 / 小修订行为方法）、`PartRepository`（新增 existsByCode 全局占用检查、按 base_no 查最新代次方法）、`PartAppService`（新增 upgradeGeneration / minorRevision / import 用例编排）、`PartType` / `LifecycleStage` 枚举、`mdm_material_part` 主表与 `mdm_material_part_history`、`mdm_outbox`（aggregate_type=PART 沿用）、`KafkaEventGatewayImpl`（topic 路由沿用）；CR-023 仅在既有类上叠加编码增强字段与行为，不新增 Controller 之外的子域基础设施。
...
### CR-023 新增技术决策（Part 零件编码增强）

| 决策 | 选择 | 备选方案 | 选型理由 |
|------|------|---------|---------|
| 零件号发号粒度 | **全局单一序列**（`mdm_material_part_seq` 单行 next_seq 行锁自增，不按 category / 前缀分段计数） | 按 category / partType / 前缀分段多计数器 | 分段计数会让分类调整 / 前缀变化牵动号段，且语义编码易冲突；全局单一计数器最简单、最稳定，对齐 CR-005 `mdm_configuration_seq` 经验 |
| 序列表复用 vs 新建 | **新建 `mdm_material_part_seq`**（与 mdm_configuration_seq 同构，单行全局计数器） | 复用 mdm_configuration_seq | 两者计数口径不同（Configuration 按 variant_code 分段、Part 全局单一），语义不可混用；新建独立序列表职责清晰 |
| 代次形态 | **2 位字母末尾位**（`AA→AB→…→AZ→BA→…→ZZ`，676 个代次空间） | 数字代次 / 1 位字母 / 独立版本号字段 | 对齐存量习惯（如 `18100020AA`）；2 位字母容量足够（676），字母代次与数字流水视觉可分；溢出 ZZ 返回 812918 |
| 互换性变更 vs 小修订 | **互换性变更升代次（新 code = 新记录、新 id）；小修订仅升 drawing_version（code 不变）** | 统一升 version 不区分 / 全部新建记录 | 互换性变更产生不可互换的新零件，必须有独立零件号供下游区分；小修订（图纸版本）不影响装配互换，复用同一零件号保持下游引用稳定 |
| 互换性判定方式 | **MDM-User 操作时显式选择**（系统不自动判定） | 系统按字段差异自动判定互换性 | 互换性是工程语义，系统无法可靠自动判定；由物料工程师在升代次 / 小修订两个入口显式选择 |
| 技术主键与业务键 | **双键分离**：surrogate `id`（BIGINT 自增技术主键，无语义、不可变）+ `code`（业务自然键，系统发号、不可变、对外展示）；内部 FK 引用 id | 仅用 code 作主键 / 仅用 id | code 含语义且会随代次升级产生新值，作内部 FK 不稳定；surrogate id 无语义稳定，承担关系引用。`mdm_material_part` 既有 id 列已是自增主键，本期明确其 surrogate 双键语义 |
| 发号来源区分方式 | **`numbering_source` 枚举 + `source_id` 对账**（MDM_GEN / PLM / IMPORT，默认 MDM_GEN），不污染 code | 给 MDM 兜底号加专属号段 / 前缀与上游号隔离 | 号段隔离会污染 code 语义且未来 PLM 切换需重编码；用 numbering_source + source_id 区分，PLM 上线后存量 MDM_GEN 号靠 source_id 认领、不重编码 |
| 软硬件前缀 | **软件件（is_software=true）加 `S` 前缀，硬件无前缀**；前缀仅作区分位，共用同一全局序列 | 软硬件各自独立序列 | 对齐存量习惯（软件 `S` 前缀）；共用全局序列避免双计数器维护成本，前缀不改变流水计数口径 |
| 分类与编码解耦 | **分类仅落 category_code，禁止编入 code / base_no** | 把分类层级编入零件号 | 语义编码会导致分类调整牵动零件号、发号冲突；分类维度由 category_code（→ MaterialCategory）独立承载 |
| N 位流水位宽 | **锁定 8 位零填充**（对齐存量 `18100020AA` 主体 8 位） | 7 位（对齐 Configuration）/ 可配置 | 存量硬件号主体为 8 位数字；锁定 8 位避免与存量冲突；溢出（> 99,999,999）返回 812919 |
| 上游 code 冲突兜底 | **沿用 CR-005 US-030 两层决策**（命中 (source_system, source_id) 幂等更新保持原 code；未命中按 code 是否全局占用决定直采或本地兜底生成 + 告警 + 监控 `mdm.material.part.code.upstream_conflict`） | 上游 code 强制直采 / 直接拒绝冲突 | 与 Configuration 上游接入范式一致，命名空间冲突不阻塞接入且可对账追溯；本期 PLM 未上线，规则预置供未来启用 |
| 手动录入兜底 | **受 `mdm:material:part:code:manual` 权限**手动指定 code（异常 / 特批，须审计），无权限返回 812920 | 完全禁止手动 / 默认开放手动 | 默认系统取号杜绝人工录入冲突；特批 / 迁移场景需兜底，用专用权限点 + 审计控制风险 |
| 代次升级 API 形态 | **独立端点 POST .../{code}/upgradeGeneration**（基于 base_no 生成新代次新记录） | 复用 create / update | 代次升级语义是"基于旧代次派生新记录"，与裸创建 / 原地更新均不同，独立端点语义清晰 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | BIGINT | Y | 主键，自增；**CR-023 明确为 surrogate 技术主键**（无语义、不可变、系统生成，承担内部外键 / 实体关系引用） |
| code | VARCHAR(64) | Y | 业务主键，全局唯一（=零件号）；**CR-023 重定义为系统发号**：形态 `[可选前缀] + 8 位零填充流水 + 2 位字母代次`（软件 `S` 前缀、硬件无前缀，如 `18100020AA` / `S1810021AA`），创建后不可变（与 CR-009 的人工录入语义被本 CR 覆盖） |
| base_no | VARCHAR(64) | Y | **CR-023 新增**：零件基础号 = code 去除末尾 2 位代次的主体部分（`[可选前缀] + 8 位流水`），用于把同一零件多个代次聚合追溯；同一零件升代次时 base_no 不变 |
| numbering_source | VARCHAR(16) | Y | **CR-023 新增**：发号来源枚举（MDM_GEN / PLM / IMPORT），默认 MDM_GEN，不可为空；靠 numbering_source + source_id 区分与对账，不污染 code |
...
| is_assembly | TINYINT(1) | N | **CR-023 新增**：是否总成件，默认 0（与 part_type 配合区分总成 / 单件） |
...
- IDX_PART_BASE_NO (base_no)：**CR-023 新增**，用于按 base_no 聚合同一零件的多代次记录、查最新代次（US-074 代次升级）
- IDX_PART_NUMBERING_SOURCE (numbering_source)：**CR-023 新增**，用于按发号来源对账（MDM_GEN / PLM / IMPORT 分布统计）

**业务约束**：
- **code 由系统发号（CR-023）**：创建时不接受调用方传入 code（DTO 不暴露 code 入参），由 PartNumberingDomainService 在同一本地事务内对 mdm_material_part_seq 行锁自增取号 → 拼接 `[前缀] + 8 位流水 + 代次 AA`；受 `mdm:material:part:code:manual` 权限的手动录入与 numbering_source=IMPORT 的存量导入为例外
- code 一经创建即不可变，更新接口 SHALL 拒绝修改 code 与 id（保留原值）
...
- (source_system, source_id) 作为上游记录的逻辑主键用于幂等校验（预留，本期不使用）
- **base_no = code 去除末尾 2 位代次（CR-023）**：base_no 与代次合成 code；同一 base_no 下不同代次为不同记录（不同 id）
- **代次升级（CR-023）**：互换性变更基于 base_no 生成下一代次（`AA→…→ZZ`）作为新 code 新记录（新 id、version=1），旧记录保留；代次溢出 ZZ 返回 812918
- **小修订（CR-023）**：不影响互换性的修订仅升 drawing_version，code / 代次 / base_no / id 不变，version 自增
- **numbering_source 不可为空（CR-023）**：取值 MDM_GEN / PLM / IMPORT，非法或为空返回 812922；MDM 后台系统发号默认 MDM_GEN
- **分类与编码解耦（CR-023）**：category_code 仅承载分类维度，禁止编入 code / base_no；category_code 变更不触发 code 重编码
- **DRAFT 物理删除不回收序号（CR-023）**：删除 DRAFT 状态 Part 不回退 mdm_material_part_seq.next_seq（序号只增不复用）
...
- **mdm_material_part_history**（CR-009 新增，Material 子域第二张历史表，沿用 `mdm_material_*` 表前缀；包含 mdm_material_part 全部业务字段 + 来源字段 + 审计字段 + 上述 5 个快照专用字段；**CR-023 起结构随主表一并包含 base_no / numbering_source / is_assembly 及系统发号 code**）
...
#### mdm_material_part_seq（Part 零件号自增序列表，CR-023）

为 Part 零件号系统发号（`[可选前缀] + 8 位零填充流水 + 2 位字母代次`）提供可靠的、与业务事务一致的序号分配能力。与 mdm_configuration_seq 同构，但**维护全局单一计数器**（单行 next_seq，**不按 category / 前缀分段**）；序号只增不复用（DRAFT 物理删除不回收）。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| seq_name | VARCHAR(32) | Y | 主键，固定单行键（如 `PART_GLOBAL`），标识全局单一计数器 |
| next_seq | BIGINT | Y | 下一个待分配流水号，默认 0；分配时执行 `UPDATE … SET next_seq = next_seq + 1`，使用更新后的值零填充至 8 位作为流水段 |
| create_by | VARCHAR(64) | Y | 创建人 |
| create_time | DATETIME | Y | 创建时间 |
| modify_by | VARCHAR(64) | Y | 修改人 |
| modify_time | DATETIME | Y | 修改时间 |
...
**主键**：PK(seq_name)
**业务约束**：
- 全局单一计数器：表内仅一行（seq_name=`PART_GLOBAL`），软硬件、总成件、外购 / 自制零件共用同一流水序列；软硬件区分靠 `S` 前缀，不改变计数口径
- 在创建 Part（系统发号路径）的同一本地事务内对该行 `UPDATE … SET next_seq = next_seq + 1` 行锁自增；事务回滚时序号一并回滚，不跳号不重复
- next_seq 单调递增，不因 Part 物理删除（DRAFT 状态）而回退；序号只增不复用（避免 history / outbox / 下游订阅出现"同 code 不同实体"歧义，对齐 CR-005）
- 当 next_seq > 99,999,999（8 位上限）时拒绝继续分配并返回错误码 812919（PART_SEQ_OVERFLOW）
- 系统生成 code 撞 UK_PART_CODE（理论不应发生）时自动重试一次序号自增，仍冲突返回 812917（PART_CODE_GEN_CONFLICT）并告警
- 首次分配时若行不存在，按"INSERT … ON DUPLICATE KEY UPDATE next_seq = next_seq + 1"或先 INSERT(next_seq=0) 再 UPDATE 幂等初始化
- 代次升级（US-074）基于已有记录的 base_no 派生新代次，**不消耗 next_seq**（base_no 沿用旧记录、仅代次位 +1）；存量导入（IMPORT）/ 上游直采（PLM）/ 手动录入采用既有 code，亦不更新 next_seq
...
### F17 - Part 零件号系统发号 / 代次升级 / 小修订 / 上游 code 冲突兜底（CR-023 核心新流程）

Part 零件号发号沿用 CR-005 F9（Configuration code 自动生成）的**序列表行锁取号 + 上游两层决策**范式，差异在于：序列表为**全局单一计数器** `mdm_material_part_seq`（不按 variant 分段），code 形态为 `[前缀] + 8 位流水 + 2 位字母代次`，并新增**代次升级（互换性变更）**与**小修订（仅升图纸版本）**两条分支。

```mermaid
sequenceDiagram
    participant Caller as Caller<br/>(MDM-User / Upstream)
    participant App as PartAppService<br/>/ IngestionAppService
    participant Num as PartNumberingDomainService
    participant SeqRepo as PartSeqRepository
    ...
    alt LOCAL 系统发号（MDM-User 新建，默认）
        Caller->>App: POST /api/mpt/mdm/material/part/v1/create<br/>body: name, categoryCode, partType, isSoftware...（无 code）
        App->>Num: 业务校验（categoryCode ACTIVE、引用完整性、effectiveFrom<=To）
        Num->>SeqRepo: UPDATE mdm_material_part_seq<br/>SET next_seq = next_seq + 1<br/>WHERE seq_name='PART_GLOBAL'
        SeqRepo-->>Num: 返回新 next_seq
        alt next_seq > 99999999
            Num-->>Caller: 812919 PART_SEQ_OVERFLOW
        end
        Num->>Num: prefix = isSoftware ? "S" : ""<br/>base_no = prefix + zeroPad(next_seq, 8)<br/>code = base_no + "AA"
        Num->>Repo: SELECT 1 FROM mdm_material_part WHERE code=?（UK 兜底）
        alt code 已存在（理论不应发生）
            Num->>SeqRepo: 重试一次 next_seq+1，重算 code
            alt 仍冲突
                Num-->>Caller: 812917 PART_CODE_GEN_CONFLICT + 告警
            end
        end
        App->>Repo: INSERT mdm_material_part<br/>（同一事务，version=1, numbering_source=MDM_GEN）
        App->>DB: INSERT mdm_material_part_history (CREATE)
        App->>Outbox: INSERT mdm_outbox<br/>(aggregate_type=PART, event_type=PartCreated)
        App-->>Caller: 200 OK { id, code, baseNo, numberingSource }

    else 代次升级（互换性变更，US-074）
        Caller->>App: POST /api/mpt/mdm/material/part/v1/{code}/upgradeGeneration
        App->>Repo: findByCode(code) → 取 base_no 与当前代次
        App->>Num: nextGeneration(当前代次)
        alt 当前代次 = ZZ
            Num-->>Caller: 812918 PART_GENERATION_OVERFLOW
        end
        Num->>Num: newCode = base_no + 新代次（如 AA→AB），base_no 不变
        App->>Repo: INSERT mdm_material_part<br/>（新记录、新 id、version=1，base_no 沿用，不消耗 next_seq）
        Note over App: 旧记录保留，由 MDM-User 按需失效
        App->>DB: INSERT mdm_material_part_history (CREATE)
        App->>Outbox: INSERT mdm_outbox (event_type=PartCreated)
        App-->>Caller: 200 OK { newId, newCode, baseNo }

    else 小修订（不影响互换性，US-075）
        Caller->>App: PUT /api/mpt/mdm/material/part/v1/{code}<br/>（仅变更 drawingVersion 等非互换性字段）
        App->>Repo: UPDATE mdm_material_part<br/>（code/代次/base_no/id 不变，drawing_version 升级，version+1）
        App->>DB: INSERT mdm_material_part_history (UPDATE)
        App->>Outbox: INSERT mdm_outbox (event_type=PartUpdated)
        App-->>Caller: 200 OK { code 不变, version+1 }

    else 上游 ingest 路径（Kafka / Feign，本期 PLM 未上线，规则预置）
        Caller->>App: ingest(sourceSystem, sourceId, sourceVersion, payload)
        Note over App: 复用 F5/F6 既有处理链路<br/>（schema → 鉴权 → 权威源 → 幂等 → 业务校验）
        App->>Repo: SELECT * WHERE source_system=? AND source_id=?
        alt 命中（同 (source_system, source_id)）
            Note over App: 第 1 层：幂等更新，本地 code/id/base_no/代次 不变
            opt 上游 payload.code != 本地 code
                App->>Log: 记录告警（上游 code 漂移）
            end
            App->>Repo: UPDATE（保留原 code，version+1）
            App->>Outbox: INSERT mdm_outbox (PartUpdated)
            App->>Log: SUCCESS, operationType=UPDATED
        else 未命中
            Note over App: 第 2 层：新增流程
            alt payload.code 为空 或 形态非法 / 长度 > 64
                App->>Num: 按 LOCAL 规则系统发号（numbering_source 置 PLM/IMPORT）
                opt 形态非法 / 超长
                    App->>Log: 记录告警
                end
            else payload.code 非空
                App->>Repo: SELECT id WHERE code = payload.code
                alt 全局未占用
                    App->>App: code = payload.code（直采，反解 base_no/代次，numbering_source=PLM/IMPORT）
                else 全局已占用
                    App->>Num: 按 LOCAL 规则兜底发号（numbering_source=MDM_GEN）
                    App->>Log: 记录 codeOverride=true, upstreamCode, finalCode
                    App->>Metric: mdm.material.part.code.upstream_conflict++
                end
            end
            App->>Repo: INSERT mdm_material_part（version=1）
            App->>Outbox: INSERT mdm_outbox (PartCreated)
            App->>Log: SUCCESS, operationType=CREATED
        end
    end
```
...
1. **全局单一序列**：`mdm_material_part_seq` 仅一行（seq_name=`PART_GLOBAL`），软硬件 / 总成 / 外购自制共用同一流水；软硬件区分仅靠 `S` 前缀，不分段计数（与 F9 按 variant_code 分段不同）。
2. **代次升级 vs 小修订**：互换性变更走 `upgradeGeneration` 端点，基于 base_no 派生新代次新记录（新 id、不消耗 next_seq）；小修订走常规 PUT，仅升 drawing_version、code 不变。互换性与否由 MDM-User 显式选择，系统不自动判定。
3. **双键发号一致性**：系统发号在同一本地事务内完成 seq 自增 + code/base_no 拼装 + 主表 INSERT；事务回滚序号一并回滚，序号只增不复用（DRAFT 物理删除不回退 next_seq）。
4. **上游 code 冲突兜底**：完全对齐 CR-005 F9 两层决策，仅监控指标更名为 `mdm.material.part.code.upstream_conflict`；本期 PLM 未上线，规则预置供 PLM 立项 CR 启用（requirements OS-33）。
5. **手动 / 导入兜底**：手动录入受 `mdm:material:part:code:manual` 权限（无权限返回 812920）；存量导入置 numbering_source=IMPORT 并按导入 code 反解 base_no 与代次；二者均不更新 next_seq。
...
| Method | Path | 权限点 | 说明 |
|--------|------|---------|------|
| POST | /api/mpt/mdm/material/part/v1/create | mdm:material:part:add | 创建零件；**code 由系统发号（CR-023 US-073）**，请求体不暴露 code 入参，响应回填 id / code / baseNo / numberingSource |
| POST | /api/mpt/mdm/material/part/v1/createWithCode | mdm:material:part:add + mdm:material:part:code:manual | **CR-023 US-080**：受权限手动指定 code 创建（异常 / 特批场景，校验全局唯一与形态合法，写审计）；无 code:manual 权限返回 812920 |
| POST | /api/mpt/mdm/material/part/v1/import | mdm:material:part:add + mdm:material:part:code:manual | **CR-023 US-080**：存量批量导入带历史 code（numbering_source=IMPORT），按导入 code 反解 base_no 与代次；仅迁移场景使用 |
| POST | /api/mpt/mdm/material/part/v1/{code}/upgradeGeneration | mdm:material:part:edit | **CR-023 US-074**：互换性变更升代次，基于 base_no 生成下一代次（AA→AB…）作为新 code 新记录新 id，旧记录保留；代次溢出 ZZ 返回 812918 |
| PUT | /api/mpt/mdm/material/part/v1/{code} | mdm:material:part:edit | 更新零件（不可修改 code / id）；**CR-023 US-075**：小修订仅升 drawing_version，code / 代次不变 |
...
> **CR-023 Service 端响应增量**：Part 全量快照 / 单点查询 / 三个批量查询（listByCategory / listByVehicleNode / listBySupplier）的响应记录均新增 `id` / `code`（系统发号）/ `baseNo` / `numberingSource` 字段；Part 事件 payload（PartCreated / PartUpdated / PartDeleted）同步携带上述字段 + `is_assembly`，供下游按 base_no 聚合多代次、按 numbering_source 对账。接口路径与 topic 不变，仅 payload / 响应体字段叠加。
...
| 812917 | PART_CODE_GEN_CONFLICT：系统发号 code 撞 UK，重试一次仍冲突（CR-023） | 409 Conflict | Material |
| 812918 | PART_GENERATION_OVERFLOW：代次溢出（超过 ZZ），互换性变更升代次被拒绝（CR-023） | 409 Conflict | Material |
| 812919 | PART_SEQ_OVERFLOW：零件号流水序号溢出（mdm_material_part_seq.next_seq > 99,999,999，8 位上限）（CR-023） | 409 Conflict | Material |
| 812920 | PART_MANUAL_CODE_FORBIDDEN：无 mdm:material:part:code:manual 权限尝试手动指定 code（CR-023） | 403 Forbidden | Material |
| 812922 | PART_NUMBERING_SOURCE_INVALID：发号来源为空或取值不在枚举范围（MDM_GEN / PLM / IMPORT）（CR-023） | 400 Bad Request | Material |

> **CR-023 错误码段位注记**：812917 ~ 812922 接续 Material 段 Part 既有 812910 ~ 812916 分配；812921 预留。监控指标 `mdm.material.part.code.upstream_conflict`（上游 code 命名空间冲突兜底计数）非错误码，记录于 mdm_ingestion_log（codeOverride=true / upstreamCode / finalCode）与 Prometheus。
...
| US-073 | §1 (CR-023 类清单 PartNumberingDomainService / PartSeqRepository), §2 (CR-023 决策), §3.1 (mdm_material_part code 系统发号 + base_no/numbering_source 列 + 业务约束), §3.6 (mdm_material_part_seq 全局单一计数器), §4 (F17 LOCAL 系统发号分支), §5.1 (Part create 系统发号), §5.5 (812917 / 812919) | Part 零件号系统发号（全局单一序列 + 前缀 + 8 位流水 + AA 代次 + UK 兜底重试） |
| US-074 | §2 (CR-023 代次升级 API 决策), §3.1 (IDX_PART_BASE_NO + 代次升级约束), §4 (F17 代次升级分支), §5.1 (POST .../{code}/upgradeGeneration), §5.5 (812918) | Part 代次升级（互换性变更 → 新 code 新记录新 id，base_no 不变，溢出 ZZ 拒绝） |
| US-075 | §2 (CR-023 互换性 vs 小修订决策), §3.1 (小修订约束), §4 (F17 小修订分支), §5.1 (PUT .../{code} 仅升 drawing_version) | Part 小修订（仅升 drawing_version，code/代次/id 不变） |
| US-076 | §1 (id surrogate 语义), §2 (CR-023 双键决策), §3.1 (id surrogate 技术主键 + code 业务自然键 + 更新拒改 id/code) | Part 双键约束与引用（surrogate id ↔ code 分离，内部 FK 引用 id） |
| US-077 | §2 (CR-023 软硬件前缀决策), §3.1 (is_assembly 列 + is_software 前缀), §4 (F17 prefix=isSoftware?"S":"") | Part 软硬件 / 总成编码区分（S 前缀 + is_assembly，共用全局序列） |
| US-078 | §2 (CR-023 分类编码解耦决策), §3.1 (category_code 解耦约束) | Part 分类与编码解耦（分类仅落 category_code，禁止编入 code） |
| US-079 | §1 (NumberingSource 枚举), §2 (CR-023 numbering_source 决策), §3.1 (numbering_source 列 + IDX_PART_NUMBERING_SOURCE), §5.5 (812922) | Part 发号来源治理与 PLM 平滑切换（MDM_GEN/PLM/IMPORT，PLM 切换预留 OS-33） |
| US-080 | §1 (PartImportCmd), §2 (CR-023 手动录入兜底决策), §5.1 (createWithCode / import 端点 + code:manual 权限), §5.5 (812920) | Part 后台录入三层模式（系统取号 / 导入带号 / 受权限手动录入） |
| US-081 | §2 (CR-023 上游冲突兜底决策), §4 (F17 上游 ingest 两层决策分支), §3.5 (mdm_ingestion_log entity_type=PART 预留 + codeOverride) | Part 上游 code 冲突兜底（沿用 CR-005 US-030 两层决策 + 监控指标） |
| US-082 | §1 (CR-023 复用既有类不重建), §3.2 (mdm_material_part_history 含 base_no/numbering_source), §3.3 (mdm_outbox aggregate_type=PART 沿用), §5.2 (Service 响应 / 事件 payload 增量字段) | Part 编码增强跨切面（事件 / 快照 / 历史 / 上游接入增量，复用既有基础设施） |
...
### Material 子域 Part 零件编码增强对现有 design 的影响（CR-023）

#### 包结构 / 模块层影响

- 无新增包、无新增子域——沿用现有扁平包结构
- 新增值对象 / 领域服务 / 仓储 / 异常：`NumberingSource` / `PartGeneration` / `PartCode` / `PartNumberingDomainService` / `PartSeqRepository` + `PartSeqRepositoryImpl` / `PartSeqPo` / `PartSeqMapper` 及 5 个发号相关异常（详见 §1 CR-023 新增类）
- 既有 `Part` 聚合根新增 base_no / numberingSource / generation 字段与升代次 / 小修订行为；`PartAppService` 新增 upgradeGeneration / minorRevision / import 用例；`PartRepository` 新增 existsByCode 与按 base_no 查最新代次方法
- API 模块 `PartCreateRequest` 移除 code 入参（系统发号）；新增 `PartGenerationUpgradeRequest` / `PartImportRequest`；`PartResponse` / `PartBriefResponse` / 快照响应新增 id / code / baseNo / numberingSource 字段

#### 数据库层影响

- 新增 1 张序列表 `mdm_material_part_seq`（全局单一计数器，对齐 mdm_configuration_seq）；Flyway 脚本 `V*_MATERIAL__create_part_seq.sql`
- `mdm_material_part` 主表 ALTER TABLE 新增 3 列：base_no（VARCHAR(64) 必填）/ numbering_source（VARCHAR(16) 必填默认 MDM_GEN）/ is_assembly（TINYINT(1) 默认 0）；新增索引 IDX_PART_BASE_NO、IDX_PART_NUMBERING_SOURCE；`mdm_material_part_history` 随主表结构同步新增上述列
- 存量数据迁移：存量 Part 记录回填 base_no（按 code 反解去末 2 位代次）、numbering_source=IMPORT（标记为存量迁移导入）、is_assembly=0；由独立数据迁移脚本承接（不在本 CR 展开搬迁工程）
- code 列语义从人工录入业务主键改为系统发号（schema 类型不变 VARCHAR(64)，仅写入路径与约束变化）

#### 错误码层影响

- 新增 8129XX 段错误码 812917 / 812918 / 812919 / 812920 / 812922 共 5 个（812921 预留），接续 Part 既有 812910 ~ 812916
- 新增 Prometheus 监控指标 `mdm.material.part.code.upstream_conflict`（上游 code 命名空间冲突兜底计数，本期 PLM 未上线不产生增量）

#### 对下游系统的影响（CR-023 新增）

- **VMD（沿用 VMD-CR-015）**：Part 本地投影副本新增 base_no / numbering_source 字段同步；下游可按 base_no 聚合同一零件的多代次记录、识别互换性版本（代次升级 = 新 code 新记录），按 PartUpdated（小修订仅升 drawing_version）区分非互换性修订
- **采购系统 / SRM / BOM（未来）**：可基于 base_no 做零件代次族追溯；BOM 引用 Part 时建议引用 surrogate id（稳定无语义），代次升级不影响历史 BOM 的 id 引用
- **PLM（未来立项 CR，OS-33）**：PLM 上线后切换为 Part 发号权威源，MDM 侧 code 转接收只读、存量 numbering_source=MDM_GEN 号靠 source_id 认领对账、不重编码；本期 design 仅预置 numbering_source 语义、上游两层 code 决策（F17）与序列表，PLM 实际接入实现留待独立 CR

#### 权限治理影响（CR-023 新增）

- 新增权限点 `mdm:material:part:code:manual`（受权限手动指定 code，用于异常 / 特批 / 存量导入场景，须留审计），与既有 `mdm:material:part:*` 六细分点 + `remove:force` 旁路点并存
- 手动录入 / 导入 / force 删除均须写审计日志（操作人 / 时间 / 目标 code / 原因说明）
...
| OQ-15 | Part 的 partType 枚举是否需要更细化的分类？ | 首期采用 RAW_MATERIAL / STANDARD_PART / CUSTOM_PART 三类（物料性质/类型维度，MDM-DSN-CR-024 重构移除 SOFTWARE/ASSEMBLY）；软硬件判定由 is_software 承载，总成判定由 is_assembly 承载，未来按业务反馈扩展（待物料工程确认） | 待确认（CR-009，requirements Q20） |
...
| OQ-21 | Part 零件号发号采用全局单一序列还是按 category / 前缀分段计数？ | 全局单一序列（`mdm_material_part_seq` 单行 next_seq 行锁自增，不分段）；软硬件区分仅靠 `S` 前缀不改变计数口径；对齐 CR-005 mdm_configuration_seq 经验 | 已确认（CR-023，requirements 决策 1） |
| OQ-22 | 代次形态用字母还是数字？置于何处？ | 末尾 2 位字母（`AA→AB→…→ZZ`，676 容量），对齐存量习惯（如 `18100020AA`）；互换性变更升代次（新 code 新记录），小修订仅升 drawing_version | 已确认（CR-023，requirements 决策 2） |
| OQ-23 | 技术主键与业务键是否分离？ | 双键分离：surrogate id（BIGINT 自增、无语义、不可变）承担内部 FK / 关系引用，code（业务自然键、系统发号、不可变）对外展示；内部关系与未来 BOM / SubstituteRelation 引用 id | 已确认（CR-023，requirements 决策 3） |
| OQ-24 | MDM 兜底发号是否靠专属号段 / 前缀与上游号隔离防冲突？ | 不靠号段隔离；通过 numbering_source（MDM_GEN / PLM / IMPORT）+ source_id 区分对账，不污染 code；上游 code 冲突沿用 CR-005 US-030 两层决策兜底 | 已确认（CR-023，requirements 决策 4） |
| OQ-25 | N 位流水位宽锁定为多少？ | 锁定 8 位零填充（对齐存量 `18100020AA` 主体 8 位）；溢出（> 99,999,999）返回 812919 | 已确认（CR-023，requirements §三/§五） |
...
| 2026-06-11 | CR-023 | Added | Material 子域 Part 零件编码增强（对 CR-009 Part 实体增强，不新建实体）：(1) 顶部 banner Material 覆盖范围追加 CR-023 + 新增 Part 零件编码增强硬约束注记（系统发号形态 / 全局单一序列 / 双键 / 代次 / numbering_source / 三层录入 / 上游冲突兜底）；(2) §1 新增 CR-023 类清单（NumberingSource / PartGeneration / PartCode 值对象 + PartNumberingDomainService 发号领域服务 + PartSeqRepository/Impl + PartSeqPo/Mapper + 5 个发号异常 + 3 个 Cmd），并标注复用既有 Part 聚合根 / Repository / AppService 不重建；(3) §2 新增 CR-023 技术决策表 12 行（全局单一序列 / 序列表新建 / 2 位字母代次 / 互换性变更 vs 小修订 / 互换性显式选择 / 双键分离 / numbering_source 对账 / 软硬件 S 前缀 / 分类编码解耦 / 8 位流水 / 上游冲突兜底 / 代次升级独立端点）；(4) §3.1 mdm_material_part 重定义 code 为系统发号 + id 明确 surrogate 语义 + 新增 base_no / numbering_source / is_assembly 列 + IDX_PART_BASE_NO / IDX_PART_NUMBERING_SOURCE 索引 + 7 条 CR-023 业务约束；§3.2 part_history 随主表同步新增列；§3.6 新增 mdm_material_part_seq 全局单一计数器序列表（对齐 mdm_configuration_seq，8 位上限溢出 812919）；(5) §4 新增 F17 流程（mermaid：LOCAL 系统发号 / 代次升级 / 小修订 / 上游 ingest 两层 code 决策 四分支 + 5 个关键设计点）；(6) §5.1 Part MPT 新增 createWithCode / import / upgradeGeneration 端点并更新 create（系统发号）/ PUT（小修订）说明；§5.2 新增 CR-023 Service 响应 / 事件 payload 增量字段注记（id/code/baseNo/numberingSource/is_assembly）；§5.5 新增错误码 812917 / 812918 / 812919 / 812920 / 812922（812921 预留）+ 监控指标注记；(7) §6 Coverage Mapping 追加 US-073 ~ US-082 共 10 行；(8) §7 Impact 新增 CR-023 影响段（包结构 / 数据库 ALTER + 序列表 + 存量回填 / 错误码 / 下游 VMD-采购-BOM-PLM / 权限治理 mdm:material:part:code:manual）；(9) §8 Open Questions 追加 OQ-21 ~ OQ-25（已确认，对应 requirements 四项锁定决策 + 8 位流水）；本期所有设计决策为已锁定结论，无待确认项 |
