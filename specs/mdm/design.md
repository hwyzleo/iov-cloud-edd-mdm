...
> **覆盖范围声明**：本 design 当前覆盖 **Product MDM 子域**（CR-001 ~ CR-005）、**Party MDM 子域**（CR-006）、**EEAD 子域**（CR-007，含 US-039 ~ US-046 车载节点字典首版能力）与 **Org 子域**（CR-008，含 US-047 ~ US-055 工厂 Plant 首版能力）。
>
...
>
> **错误码段位说明**：本 design §5.5 错误码表当前段位 `807XXX`（CR-001 ~ CR-006 历史已落，共 17 个错误码）与 `812XXX`（CR-007 新分配，含 812001 / 812002 / 812003）**共存**；段位历史漂移由 requirements §7 Q12 跟踪，未来通过独立 CR 统一对齐。
...
|------|------|------|------|
| id | BIGINT | Y | 主键，自增 |
| aggregate_type | VARCHAR(32) | Y | 聚合类型（BRAND / SERIES / PLATFORM / MODEL / VARIANT / CONFIGURATION / OPTION_FAMILY / OPTION_CODE / SUPPLIER / VEHICLE_NODE / **PLANT**）；CR-007 新增 VEHICLE_NODE 取值，CR-008 新增 PLANT 取值 |
...
|------|------|------|------|
| id | BIGINT | Y | 主键，自增 |
| entity_type | VARCHAR(16) | Y | 实体类型（BRAND / SERIES / PLATFORM / MODEL / VARIANT / CONFIGURATION / OPTION_FAMILY / OPTION_CODE / SUPPLIER / VEHICLE_NODE / **PLANT**）；CR-007 预留 VEHICLE_NODE 取值，CR-008 预留 PLANT 取值供未来 Org 开启上游接入路径时使用，本期不写入 |
...
### 5.5 错误码表（段位 807XXX / 812XXX / 813XXX 共存，详见 requirements §7 Q12）
...
> **段位历史漂移说明（CR-007 / CR-008）**：MDM 服务规范段位为 812xxx，但 Product MDM / Party MDM 历史 CR-001 ~ CR-006 已使用 807xxx 段（共 17 个错误码），EEAD 子域本期使用 812xxx 段（3 个），Org 子域本期使用 813xxx 段（4 个 + 4 个预留）。三段共存为历史遗留，未来由独立 CR 统一对齐（详见 requirements §7 Q12）。
