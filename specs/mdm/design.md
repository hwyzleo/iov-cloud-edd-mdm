...
> **覆盖范围声明**：本 design 当前覆盖 **Product MDM 子域**（CR-001 ~ CR-005）、**Party MDM 子域**（CR-006）、**EEAD 子域**（CR-007，含 US-039 ~ US-046 车载节点字典首版能力）、**Org 子域**（CR-008，含 US-047 ~ US-055 工厂 Plant 首版能力）与 **Material 子域**（CR-009，含 US-056 ~ US-070 物料品类 MaterialCategory + 零件 Part 首版能力）。
>
...
### 5.5 错误码表（段位 807XXX / 812XXX / 813XXX / 814XXX 共存，详见 requirements §7 Q12）
...
| **812003** | **VEHICLE_NODE_HAS_DOWNSTREAM_REF：车载节点存在下游引用，删除被拒绝（CR-007 EEAD 子域，含 referenceCount + 前 10 条引用样本）** | **409 Conflict** |
| **813001** | **PLANT_NOT_EXIST：工厂不存在（CR-008 Org 子域）** | **404 Not Found** |
| **813002** | **PLANT_CODE_EXIST：工厂 code 已存在（CR-008 Org 子域）** | **409 Conflict** |
| **813003** | **PLANT_HAS_DOWNSTREAM_REF：工厂存在下游引用，删除被拒绝（CR-008 Org 子域，含 referenceCount + 前 10 条引用样本）** | **409 Conflict** |
| **813004** | **PLANT_EFFECTIVE_PERIOD_INVALID：工厂生效期非法（effective_from > effective_to）（CR-008 Org 子域）** | **400 Bad Request** |
| 813010 | PLANT_UPSTREAM_SCHEMA_INVALID：上游消息 schema 非法（CR-008 预留，本期不使用） | 400 Bad Request |
| 813011 | PLANT_UPSTREAM_AUTH_FAILED：上游来源鉴权失败（CR-008 预留，本期不使用） | 401 Unauthorized |
| 813012 | PLANT_UPSTREAM_VERSION_CONFLICT：同版本冲突（CR-008 预留，本期不使用） | 409 Conflict |
| 813013 | PLANT_UPSTREAM_NON_AUTHORITY：非权威源写入被拒绝（CR-008 预留，本期不使用） | 403 Forbidden |
| **814001** | **MATERIAL_CATEGORY_NOT_EXIST：物料品类不存在（CR-009 Material 子域）** | **404 Not Found** |
| **814002** | **MATERIAL_CATEGORY_CODE_EXIST：物料品类 code 已存在（CR-009 Material 子域）** | **409 Conflict** |
| **814003** | **MATERIAL_CATEGORY_HAS_CHILDREN：物料品类存在子项或被 Part 引用，删除被拒绝（CR-009 Material 子域）** | **409 Conflict** |
| **814004** | **MATERIAL_CATEGORY_EFFECTIVE_PERIOD_INVALID / PART_EFFECTIVE_PERIOD_INVALID：生效期非法（effective_from > effective_to）（CR-009 Material 子域）** | **400 Bad Request** |
| **814005** | **MATERIAL_CATEGORY_PARENT_NOT_EXIST：父品类不存在（CR-009 Material 子域）** | **400 Bad Request** |
| **814006** | **MATERIAL_CATEGORY_LOOP_DETECTED：物料品类层级形成环路（CR-009 Material 子域）** | **400 Bad Request** |
| **814010** | **PART_CODE_EXIST：零件 code 已存在（CR-009 Material 子域）** | **409 Conflict** |
| **814011** | **PART_CATEGORY_INVALID：零件 categoryCode 指向不存在或非 ACTIVE 的 MaterialCategory（CR-009 Material 子域）** | **400 Bad Request** |
| **814012** | **PART_VEHICLE_NODE_INVALID：零件 vehicleNodeCode 指向不存在的 VehicleNode（CR-009 Material 子域）** | **400 Bad Request** |
| **814013** | **PART_SUPPLIER_INVALID：零件 supplierCode 指向不存在的 Supplier（CR-009 Material 子域）** | **400 Bad Request** |
| **814014** | **PART_SUBSTITUTE_INVALID：零件 substitutePartCode 指向不存在或指向自身（CR-009 Material 子域）** | **400 Bad Request** |
