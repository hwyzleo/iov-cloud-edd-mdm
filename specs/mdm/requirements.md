...
| **Material MDM** | 管"用什么造"——物料、零件、品类等物料主数据 | CR-009 本期纳入（Part + MaterialCategory 首版）；CR-023 增强 Part 零件号系统化发号治理 |
...
> CR-023 本期纳入：对 CR-009 已落地的 Part 实体做**编码能力增强**（不新建实体）——将零件号（partNo / code）从人工录入升级为**系统化发号治理**，引入全局单一序列发号、字母代次（互换性变更升代）、技术主键与业务键分离（surrogate id ↔ code 双键）、发号来源治理（numbering_source）四项核心能力，对齐 CR-005 Configuration 的发号经验（序列表 + 系统取号 + DRAFT 不回收）。详见 §2 G22、域 16（US-073 ~ US-082）与 §5 约束。
...
- **G22**：将 Part 零件号纳入**系统化发号治理**（CR-023，对 CR-009 Part 实体的编码能力增强，不新建实体）——零件号 `code` 由系统按"`[可选前缀] + N 位零填充流水 + 2 位字母代次`"统一发号（全局单一序列、创建后不可变、DRAFT 物理删除不回收序号），引入技术主键 surrogate `id` 与业务键 `code` 分离的双键模型、字母代次（互换性变更升代次=新记录、小修订仅升图纸版本）、`base_no` 多代次聚合追溯、`numbering_source` 发号来源治理（MDM_GEN / PLM / IMPORT）；本期 PLM 未上线由 MDM 兜底发号（MDM_GEN），并为未来 PLM 上线后平滑切换为发号权威源预留扩展语义与字段
...
| Part Type | 零件类型 | Part 的分类属性枚举（物料性质/类型维度）。取值含原材料（RAW_MATERIAL）、标准件（STANDARD_PART）、定制件（CUSTOM_PART）；软硬件判定由 is_software 布尔字段承载，总成判定由 is_assembly 布尔字段承载 |
...
| Part No | 零件号 | CR-023 纳入。即 Part 的业务自然键 `code`，对外展示与查询用，形态为 `[可选前缀] + N 位零填充流水 + 2 位字母代次`（如硬件 `18100020AA`、软件 `S1810021AA`）；由系统统一发号、全局唯一、创建后不可变 |
| Base No | 零件基础号 | CR-023 纳入。指零件号去除末尾 2 位字母代次后的主体部分（`[可选前缀] + N 位流水`），用于把同一零件的多个代次聚合追溯；同一零件升代次时 base_no 不变 |
| Generation | 代次 | CR-023 纳入。零件号末尾 2 位字母变更位，从 `AA` 起步按 `AA→AB→…→AZ→BA→…→ZZ` 递增。**互换性变更（影响装配/功能互换）→ 升代次（新 code = 新记录）**；不影响互换性的小修订仅升图纸版本（drawing_version），代次与 code 不变。等价于行业惯例中的"零件版本号 / 互换性版本" |
| Numbering Source | 发号来源 | CR-023 纳入。标识零件号的发号来源枚举：MDM_GEN（MDM 兜底自动生成）/ PLM（PLM 发号同步）/ IMPORT（存量迁移导入带号），默认 MDM_GEN。用于区分与对账，不污染 `code` 本身 |
| Surrogate Id | 技术主键 | CR-023 纳入。Part 的无语义、不可变、系统生成的代理主键（`id`），承担内部外键 / 实体关系引用；与业务自然键 `code` 分离（双键模型），所有内部 FK 与未来关系（BOM / SubstituteRelation）一律引用 `id` 而非 `code` |
| Part Sequence | 零件号序列 | CR-023 纳入。独立序列表 `mdm_material_part_seq`，维护**全局单一计数器**（next_seq 单行、行锁自增），不按 category / 前缀分段计数；对齐 CR-005 `mdm_configuration_seq` 模式，DRAFT 物理删除不回收序号 |
...
> - ✅ partType: 枚举，零件类型（物料性质/类型维度：RAW_MATERIAL / STANDARD_PART / CUSTOM_PART；软硬件判定由 is_software 承载，总成判定由 is_assembly 承载）
...
### 域 16: Material MDM - Part 零件编码增强（CR-023）

> **本域定位**：对 CR-009 已落地的 Part 实体（域 15 / US-059）做**编码能力增强**，不新建实体、不引入新子域，命名空间完全沿用 Material 子域（表前缀 `mdm_material_*`、Kafka topic `mdm.material.*`、Feign 路径 `/api/service/mdm/material/v1/**`、权限点 `mdm:material:*`、Java 类名前缀 `Part*`）。本域 US 与域 15 各 US 合并生效；凡与 CR-009 US-059 既有 code 行为冲突处（code 由人工录入），以本域 US-073 为准（系统发号覆盖人工录入）。

> **Part 字段扩展（CR-023）**（在 US-059 既有 Part 字段表基础上新增 / 重定义以下字段，✅必填 / ⭕可选标注沿用，与既有字段合并展示）：
>
> 编码与双键属性
> - ✅ id: surrogate 技术主键，无语义、不可变、系统生成（雪花 / 序列均可，由 design 阶段锁定）；承担内部外键 / 实体关系引用
> - ✅ code: 零件号（partNo），业务自然键，全局唯一，**系统生成、创建后不可变**；形态 = `[可选前缀] + N 位零填充流水 + 2 位字母代次`（N 位宽由 design 阶段锁定，对齐存量 8 位习惯，如 `18100020AA`；软件件以 `S` 前缀，如 `S1810021AA`）。**重定义**：CR-009 US-059 中 code 为人工录入业务主键，CR-023 起改为系统发号（与 CR-005 Configuration code 同范式）
> - ✅ base_no: VARCHAR(64)，零件基础号（去除末尾 2 位代次的主体部分 = `[可选前缀] + N 位流水`），用于把同一零件的多个代次聚合追溯；同一零件升代次时 base_no 不变
> - ✅ numbering_source: 枚举 VARCHAR(16)，发号来源（MDM_GEN / PLM / IMPORT），默认 MDM_GEN，不可为空
>
> 软硬件 / 总成属性
> - ⭕ part_type: 枚举，零件类型（物料性质/类型维度：RAW_MATERIAL / STANDARD_PART / CUSTOM_PART；软硬件判定由 is_software 承载，总成判定由 is_assembly 承载）
> - ⭕ is_assembly: BOOLEAN，是否总成件
>
> 分类与图纸（分类维度解耦，不进入 code）
> - ⭕ category_code: VARCHAR(64)，物料品类编码（→ MaterialCategory.code，沿用 CR-009；分类维度解耦，**不进入 code**）
> - ⭕ drawing_no: VARCHAR(64)，图纸号（沿用 CR-009 drawingNo）
> - ⭕ drawing_version: VARCHAR(32)，图纸版本（沿用 CR-009 drawingVersion；小修订升此字段，不升代次）
>
> 复用既有治理字段（与 CR-009 / 各子域对齐，不重复定义）：source_system / source_id / source_version / ingestion_channel / ingestion_time / source_payload_hash / version / effective_from / effective_to / status / create_by / create_time / modify_by / modify_time / row_version / row_valid
>
> ✅ = 必填，⭕ = 可选

#### US-073: Part 零件号系统发号

**As a** MDM-User, **I want** 创建 Part 时由系统自动发号生成零件号 code（不再人工录入），**so that** 统一发号规则、避免人工录入冲突与语义编码。

> **发号规则（CR-023，对齐 CR-005 mdm_configuration_seq 范式）**：零件号流水使用独立序列表 `mdm_material_part_seq` 的**全局单一计数器**（next_seq 单行、行锁自增），**不按 category / 前缀分段计数**；code = `[可选前缀] + N 位零填充流水 + 2 位字母代次`，新建零件代次从 `AA` 起步；base_no = code 去除末尾 2 位代次的主体部分；numbering_source 默认 MDM_GEN。DRAFT 状态物理删除时**不回收序号**（避免 history / outbox / 下游订阅出现"同 code 不同实体"歧义）。

**Acceptance Criteria** (EARS 语法):

- WHEN MDM-User 创建 Part THEN THE SYSTEM SHALL 在同一本地事务内执行：(1) 对 mdm_material_part_seq 单行 next_seq 行锁自增；(2) 按 US-077 规则确定前缀（软件件 `S`、硬件无前缀），拼接 code = `[前缀]` + 自增序号零填充至 N 位 + 代次 `AA`；(3) 计算并落 base_no = `[前缀]` + N 位零填充流水；(4) 持久化 Part 记录并设置 version=1、numbering_source=MDM_GEN，自动填充 create_by、create_time、modify_by、modify_time
- WHEN MDM-User 创建 Part 且请求体携带 code 字段 THEN THE SYSTEM SHALL 静默忽略该字段并仍按系统规则生成 code（DTO 不暴露 code 入参，REST 风格）；**例外**：受 `mdm:material:part:code:manual` 权限的手动录入与 `numbering_source=IMPORT` 的存量导入按 US-080 处理
- WHEN 系统生成 code 在 mdm_material_part 表已存在（理论不应发生，UK 兜底）THEN THE SYSTEM SHALL 自动重试一次序号自增；仍冲突则返回错误码 812917（PART_CODE_GEN_CONFLICT）并告警
- WHEN 序号自增后流水超出 N 位零填充上限 THEN THE SYSTEM SHALL 返回错误码 812919（PART_SEQ_OVERFLOW）并拒绝创建
- WHEN MDM-User 删除 Part 且 status=DRAFT THEN THE SYSTEM SHALL 物理删除记录，**不回退 mdm_material_part_seq.next_seq**（序号只增不复用）
- WHEN 创建接口返回响应 THEN THE SYSTEM SHALL 在响应体中回填本次生成的 id / code / base_no / numbering_source 字段

#### US-074: Part 代次升级（互换性变更）

**As a** MDM-User, **I want** 对发生互换性变更的零件升级代次, **so that** 同一零件的不同互换性版本可被独立标识并聚合追溯。

**Acceptance Criteria** (EARS 语法):

- WHEN MDM-User 对某 Part 发起互换性变更（代次升级）THEN THE SYSTEM SHALL 基于该 Part 的 base_no 计算下一代次（`AA→AB→…→AZ→BA→…→ZZ`），以 `code = base_no + 新代次` 创建**新记录**（新 id、version=1、numbering_source 沿用原记录或按发号来源规则），新记录 base_no 与原记录一致
- WHEN 代次升级生成新记录 THEN THE SYSTEM SHALL 保留原代次记录（不删除、不改 code、不改 id），由 MDM-User 按需失效（status=INACTIVE）原记录
- WHEN 代次已达 `ZZ` 仍请求升代次 THEN THE SYSTEM SHALL 返回错误码 812918（PART_GENERATION_OVERFLOW）并拒绝
- WHEN 代次升级新记录持久化 THEN THE SYSTEM SHALL 写 history、写 outbox（eventType=PartCreated，按 US-064 / US-082）
- THE SYSTEM SHALL NOT 在互换性变更时复用旧记录的 id 或原地修改旧 code（互换性变更语义即"新 code = 新记录"）

#### US-075: Part 小修订（图纸版本）

**As a** MDM-User, **I want** 对不影响互换性的小修订仅升级图纸版本, **so that** 同一零件的非互换性修订不产生新零件号、保持下游引用稳定。

**Acceptance Criteria** (EARS 语法):

- WHEN MDM-User 对 Part 发起不影响互换性的小修订 THEN THE SYSTEM SHALL 仅升级 drawing_version，**code / 代次 / base_no / id 保持不变**，自增 version、写 history、写 outbox（eventType=PartUpdated）
- WHEN 小修订仅变更 drawing_version THEN THE SYSTEM SHALL NOT 触发代次升级、SHALL NOT 生成新记录
- THE SYSTEM SHALL 在 design 阶段明确"互换性变更"与"小修订"由 MDM-User 在操作时显式选择（系统不自动判定互换性），互换性变更走 US-074、小修订走本 US

#### US-076: Part 双键约束与引用（surrogate id ↔ code）

**As a** System, **I want** Part 维护技术主键 id 与业务键 code 分离的双键模型, **so that** 内部关系引用稳定无语义、对外展示用业务自然键。

**Acceptance Criteria** (EARS 语法):

- THE SYSTEM SHALL 为 Part 维护 surrogate `id`（无语义、不可变、系统生成）作为技术主键，承担内部外键 / 实体关系引用
- THE SYSTEM SHALL 保证 `code`（零件号）全局唯一、创建后不可变，仅作业务自然键 + 对外展示与查询键
- THE SYSTEM SHALL 对所有内部 FK 与未来新增关系（如 BOM / SubstituteRelation）一律引用 `id`，不引用 `code`
- WHEN MDM-User 更新 Part THEN THE SYSTEM SHALL 忽略入参中的 `id` 与 `code`（保留原值不变）
- THE SYSTEM SHALL 在 design 阶段明确：CR-009 既有 `substitutePartCode` 字段本期沿用现状（仍以 code 标注一对一替代关系），其向 id 引用的迁移随 OS-26 SubstituteRelation 独立实体立项时统一处理，不在 CR-023 重构

#### US-077: Part 软硬件 / 总成编码区分

**As a** MDM-User, **I want** 通过 part_type 与 is_assembly 区分软硬件 / 总成 / 单件并在发号时区分软硬件前缀, **so that** 软硬件、总成件、单件共用统一编码骨架且可识别。

**Acceptance Criteria** (EARS 语法):

- THE SYSTEM SHALL 通过 `part_type`（物料性质/类型维度：RAW_MATERIAL / STANDARD_PART / CUSTOM_PART）与 `is_assembly`（BOOLEAN）区分总成件 / 单件
- WHEN 系统为软件件（is_software=true）发号 THEN THE SYSTEM SHALL 在 code 与 base_no 前加 `S` 前缀
- WHEN 系统为硬件件发号 THEN THE SYSTEM SHALL 不加前缀（code 与 base_no 直接以 N 位流水起头）
- THE SYSTEM SHALL 保证软硬件、总成件、外购 / 自制零件共用同一全局序列（US-073），前缀仅作软硬件区分位、不改变流水计数口径
- WHEN MDM-User 查询 Part 列表 THEN THE SYSTEM SHALL 支持按 part_type、is_assembly 过滤（与 US-059 列表过滤合并）

#### US-078: Part 分类与编码解耦

**As a** System, **I want** 零件分类仅落在 category_code 而不编入 code, **so that** 分类调整不影响零件号、避免语义编码导致的发号冲突。

**Acceptance Criteria** (EARS 语法):

- THE SYSTEM SHALL 仅将零件分类维护在 `category_code`（→ MaterialCategory.code，沿用 CR-009 引用完整性校验，见 US-060）
- THE SYSTEM SHALL NOT 将分类层级 / 品类编码编入 `code` 或 `base_no`；code 仅由 `[可选前缀] + N 位流水 + 2 位字母代次` 构成，不含任何分类语义
- WHEN MDM-User 变更 Part 的 category_code THEN THE SYSTEM SHALL 仅更新 category_code 字段（按 US-060 校验新值有效且 ACTIVE），**不触发 code 重编码、不变更 base_no / 代次**

#### US-079: Part 发号来源治理与 PLM 平滑切换

**As a** System, **I want** 通过 numbering_source 标记零件号发号来源并为未来 PLM 上线预留平滑切换路径, **so that** MDM 兜底发号与未来 PLM 发号权威源可对账切换、不重编码。

**Acceptance Criteria** (EARS 语法):

- THE SYSTEM SHALL 在 Part 主表及 history 表维护 `numbering_source` 字段（枚举 MDM_GEN / PLM / IMPORT，默认 MDM_GEN，不可为空）
- WHEN MDM-User 在 MDM 后台由系统发号创建 Part THEN THE SYSTEM SHALL 设置 numbering_source=MDM_GEN
- WHEN 存量迁移导入携带历史 code 创建 Part THEN THE SYSTEM SHALL 设置 numbering_source=IMPORT（见 US-080）
- WHEN 写入 Part 且 numbering_source 为空或取值不在枚举范围 THEN THE SYSTEM SHALL 返回错误码 812922（PART_NUMBERING_SOURCE_INVALID）并拒绝
- THE SYSTEM SHALL 通过 `numbering_source` + `source_id` 区分与对账各发号来源，**不在 `code` 本身加专属号段 / 前缀与上游号区分**（MDM 兜底自动发号不靠号段隔离防冲突，靠 US-073 UK 兜底 + US-081 冲突兜底）
- THE SYSTEM SHALL 在 design 阶段明确 PLM 平滑切换路径（本期 PLM 未上线、不实现）：未来 PLM 上线后发号源切至 PLM、MDM 侧 code 转为接收只读、存量 numbering_source=MDM_GEN 的号靠 source_id 认领对账、不重编码；PLM 实际接入实现留待独立 CR 立项（见 OS-33）

#### US-080: Part 后台录入模式（三层发号策略）

**As a** MDM-User, **I want** 后台录入零件支持系统取号 / 批量导入带号 / 受权限手动录入三层模式, **so that** 默认系统发号、迁移场景可带号、异常特批可受控手动指定。

**Acceptance Criteria** (EARS 语法):

- THE SYSTEM SHALL 默认采用**系统取号（非手动输入）**：后台新建零件不暴露 code 录入框，由系统按 US-073 发号，numbering_source=MDM_GEN
- WHEN 批量导入存量零件且携带历史 code（numbering_source=IMPORT）THEN THE SYSTEM SHALL 允许采用导入的历史 code 入库（仅限迁移 / 存量导入场景），并按导入 code 反解 base_no 与代次
- WHEN 持有 `mdm:material:part:code:manual` 权限的 MDM-User 在异常 / 特批场景手动指定 code THEN THE SYSTEM SHALL 允许手动录入 code（校验全局唯一与形态合法），并记录审计日志（操作人、时间、目标 code、原因说明）
- WHEN 不持有 `mdm:material:part:code:manual` 权限的用户尝试手动指定 code THEN THE SYSTEM SHALL 返回错误码 812920（PART_MANUAL_CODE_FORBIDDEN）并拒绝（回退为系统取号）
- WHEN 导入或手动录入的 code 与 mdm_material_part 表已存在记录冲突 THEN THE SYSTEM SHALL 返回错误码 812910（PART_CODE_EXIST）并拒绝

#### US-081: Part 上游 code 冲突兜底

**As an** Upstream-System, **I want** 上游（PLM / IMPORT）携带 code 推送 Part 且与 MDM 全局已占用 code 冲突时由 MDM 兜底发号入库, **so that** 命名空间冲突不阻塞接入且可对账追溯（对齐 CR-005 US-030 模式）。

**Acceptance Criteria** (EARS 语法):

- THE SYSTEM SHALL 复用 US-013 / US-014 已建立的上游接入处理链路（schema 校验 → 来源鉴权 → 权威源校验 → 幂等校验 → 业务校验 → upsert + history + outbox）
- WHEN 上游推送 Part 主数据 THEN THE SYSTEM SHALL 按以下两层规则决定本地 code（对齐 CR-005 US-030）：
    - **第 1 层：幂等更新**——按 (source_system, source_id) 在本地查找；命中则走更新流程，本地 code / id / base_no / 代次保持不变（即便上游本次 payload 的 code 与本地不同，亦不改 code，仅记录告警提示上游 code 漂移）
    - **第 2 层：未命中 → 进入新增流程**：
        - 若上游 payload 未携带 code → 由系统按 US-073 规则生成（numbering_source 按上游来源置 PLM 或 IMPORT）
        - 若上游 payload 携带 code 且该 code 在 MDM 全局**未被占用** → 直接采用上游 code 入库（numbering_source=PLM / IMPORT，反解 base_no 与代次）
        - 若上游 payload 携带 code 且该 code 在 MDM 全局**已被占用**（无论被占记录来源）→ 视为 **code 命名空间冲突**，本地按 US-073 规则生成新 code 兜底入库（numbering_source=MDM_GEN），记录告警日志、累加监控指标 `mdm.material.part.code.upstream_conflict`，并在 mdm_ingestion_log 记录 codeOverride=true 与 upstreamCode / finalCode
- WHEN 上游推送的 Part code 形态非法或长度超过 64 字符 THEN THE SYSTEM SHALL 视同上游未携带 code，由系统按 US-073 规则生成本地 code，并在 mdm_ingestion_log 记录告警
- THE SYSTEM SHALL 在 mdm_ingestion_log 中记录每一次 Part 上游接入处理结果（见 US-018），entityType 扩展 PART 取值

#### US-082: Part 编码增强的跨切面能力（事件 / 快照 / 历史 / 上游接入增量）

**As a** System, **I want** Part 编码增强字段纳入既有事件发布 / 全量快照 / 历史版本 / 上游接入基础设施, **so that** 下游可同步 code / 代次 / 双键 / 发号来源相关增量，复用 Material 子域既有基础设施。

**Acceptance Criteria** (EARS 语法):

- WHEN Part 发生创建（含代次升级新记录）、更新（含小修订）、失效、删除操作 THEN THE SYSTEM SHALL 复用 US-064 事件发布机制（Kafka topic `mdm.material.part.event`、eventType PartCreated / PartUpdated / PartDeleted），并在 payload 中包含编码增强字段：id / code / base_no / numbering_source / part_type / is_assembly
- WHEN Part 历史版本快照写入 THEN THE SYSTEM SHALL 复用 US-063 机制，在 mdm_material_part_history 快照中保留 id / code / base_no / numbering_source / drawing_version 字段
- WHEN Service-Caller 调用 Part 全量快照 / 单点查询 / 批量查询接口（US-065）THEN THE SYSTEM SHALL 在响应记录中包含 id / code / base_no / numbering_source 字段
- WHEN 上游通过 Kafka / Feign 推送 Part THEN THE SYSTEM SHALL 复用 US-066 数据来源记录机制并叠加 US-081 的 code 决策规则
- THE SYSTEM SHALL 复用 Product MDM / Party MDM / EEAD / Org / Material 已建立的 Outbox / Ingestion Log 基础设施，仅在 payload / 快照 / 历史字段上叠加编码增强增量，不新增 topic、不新增审计表
...
- **Part 零件号系统化发号（CR-023）**：Part 的 `code`（零件号）不接受调用方传入，由系统按 `[可选前缀] + N 位零填充流水 + 2 位字母代次` 统一发号（软件件 `S` 前缀、硬件无前缀，N 位宽由 design 阶段锁定）；流水使用独立序列表 `mdm_material_part_seq` 的**全局单一计数器**（next_seq 单行、行锁自增、**不按 category / 前缀分段计数**），行为对齐 CR-005 `mdm_configuration_seq`（行锁自增、DRAFT 物理删除不回收序号、UK 兜底重试一次仍冲突返回 812917）；code 全局唯一、创建后不可变、序号只增不复用
- **Part 代次（CR-023）**：代次为零件号末尾 2 位字母，从 `AA` 起步按 `AA→AB→…→AZ→BA→…→ZZ` 递增；**互换性变更 → 升代次（新 code = 新记录、新 id、保留旧记录）**；**不影响互换性的小修订 → 仅升 drawing_version，code / 代次不变**；互换性与否由 MDM-User 在操作时显式选择，系统不自动判定；代次溢出（超过 `ZZ`）返回 812918
- **Part 双键模型（CR-023）**：引入无语义、不可变、系统生成的 surrogate `id` 作为技术主键，承担内部外键 / 实体关系引用；`code` 仅作业务自然键 + 对外展示；所有内部 FK 与未来关系（BOM / SubstituteRelation）一律引用 `id` 不引用 `code`；更新接口忽略入参中的 `id` / `code`（保留原值）。既有 `substitutePartCode` 沿用 CR-009 现状（引用 code），其 id 化迁移随 OS-26 立项统一处理
- **Part 分类与编码解耦（CR-023）**：分类维度仅落 `category_code`（→ MaterialCategory），**禁止把分类层级编入 `code` / `base_no`**；category_code 变更不触发 code 重编码
- **Part 发号来源治理（CR-023）**：`numbering_source` 枚举（MDM_GEN / PLM / IMPORT，默认 MDM_GEN，不可为空）标记发号来源，靠 `numbering_source` + `source_id` 区分与对账，**不在 `code` 加专属号段与上游号区分**；本期 PLM 未上线由 MDM 兜底发号（MDM_GEN），未来 PLM 上线后切换为发号权威源（MDM 侧 code 转接收只读、存量 MDM_GEN 号靠 source_id 认领、不重编码），PLM 实际接入实现留待独立 CR 立项（见 OS-33）
- **Part 后台录入三层模式（CR-023）**：默认系统取号（不暴露 code 录入框）；批量导入带号场景置 numbering_source=IMPORT（仅迁移 / 存量）；受 `mdm:material:part:code:manual` 权限的手动录入兜底（异常 / 特批场景，须留审计），无该权限手动指定 code 返回 812920
- **Part 上游 code 冲突兜底（CR-023）**：上游接入沿用 US-013 / US-014 链路与 CR-005 US-030 两层 code 决策范式——命中 (source_system, source_id) 走幂等更新保持原 code；未命中按 code 是否被全局占用决定直采上游 code 或本地按 US-073 兜底生成（占用时 numbering_source=MDM_GEN，记告警、累加 `mdm.material.part.code.upstream_conflict`、ingestion_log 记 codeOverride=true 与 upstreamCode / finalCode）
- **Material 子域错误码段位**：Material 子域分配 **8129XX 段**（与 Product 的 8121XX、EEAD 的 8123XX、Org 的 8125XX、Party 的 8127XX 错开）：812901 ~ 812906（MaterialCategory）/ 812910 ~ 812916（Part，CR-009）/ 812917（PART_CODE_GEN_CONFLICT，系统发号撞 UK 重试仍冲突）/ 812918（PART_GENERATION_OVERFLOW，代次溢出 ZZ）/ 812919（PART_SEQ_OVERFLOW，流水序号溢出 N 位上限）/ 812920（PART_MANUAL_CODE_FORBIDDEN，无权限手动指定 code）/ 812922（PART_NUMBERING_SOURCE_INVALID，发号来源为空或非法）；812921 预留
...
| OS-33 | PLM 作为 Part 发号权威源的实际接入实现（订阅 PLM 推送 topic、暴露 `POST /api/upstream/mdm/material/part/v1/ingest`、发号源切至 PLM 后 MDM 侧 code 转接收只读、存量 numbering_source=MDM_GEN 号认领对账） | CR-023 仅预留扩展语义与字段（numbering_source / source_id / 双键），本期 PLM 未上线由 MDM 兜底发号（MDM_GEN）；PLM 实际接入与权威源切换灰度留待 PLM 立项 CR（沿用 OS-27 Material 上游接入路径不开启的总约束） |
...
| Q20 | Part 的 partType 枚举是否需要更细化的分类？如区分原材料 / 标准件 / 定制件 / 软件件 / 总成件 / 电子件 / 机械件等 | US-059 | **已决议：首期 3 类（MDM-DSN-CR-024 重构）**。采用 RAW_MATERIAL / STANDARD_PART / CUSTOM_PART（物料性质/类型维度）；软硬件判定由 is_software 布尔字段承载，总成判定由 is_assembly 布尔字段承载，未来按业务反馈扩展 |
...
| 2026-06-11 | CR-023 | Added | 对 CR-009 Material 子域 Part 实体做**零件编码能力增强**（不新建实体、不引入新子域，命名空间完全沿用 Material：表前缀 `mdm_material_*`、topic `mdm.material.*`、Feign `/api/service/mdm/material/v1/**`、权限点 `mdm:material:*`、类名前缀 `Part*`）：(1) §1 Overview 子域表 Material 落地状态追加 CR-023 编码增强，Material MDM 子域段落新增 CR-023 范围说明；(2) §2 新增 G22（Part 零件号系统化发号治理：全局单一序列发号 + 字母代次 + 双键 + numbering_source 发号来源 + PLM 平滑切换预留）；(3) §3 Glossary 新增 Material 术语 6 条（Part No / Base No / Generation 代次 / Numbering Source / Surrogate Id / Part Sequence）；(4) §4 新增"域 16: Material MDM - Part 零件编码增强"含 US-073 ~ US-082（系统发号 / 代次升级互换性变更 / 小修订图纸版本 / 双键约束与引用 / 软硬件总成编码区分 / 分类与编码解耦 / 发号来源治理与 PLM 平滑切换 / 后台录入三层模式 / 上游 code 冲突兜底 / 编码增强跨切面），含 CR-023 Part 字段扩展块（新增 id / base_no / numbering_source / is_assembly，重定义 code 为系统发号）；(5) §5 业务约束新增 Part 系统化发号（对齐 CR-005 mdm_material_part_seq 全局单一计数器、DRAFT 不回收）、代次、双键模型、分类编码解耦、发号来源治理、后台录入三层模式、上游 code 冲突兜底，并补全 Material 子域错误码段位说明；(6) §6 新增 OS-33（PLM 实际接入实现留待 PLM 立项 CR）；BOM / SubstituteRelation 沿用 OS-25 / OS-26 不在本期；(7) 错误码 812917（PART_CODE_GEN_CONFLICT）/ 812918（PART_GENERATION_OVERFLOW）/ 812919（PART_SEQ_OVERFLOW）/ 812920（PART_MANUAL_CODE_FORBIDDEN）/ 812922（PART_NUMBERING_SOURCE_INVALID）按需新增，812921 预留；(8) 新增权限点 `mdm:material:part:code:manual`（受权限手动录入 code）；本期所有设计决策为已锁定结论，无 Open Question 抛出 |
