...
- WHEN MDM-User 创建 VehicleNode 且 nodeCode 已存在 THEN THE SYSTEM SHALL 返回错误码 812002（VEHICLE_NODE_CODE_EXIST）并拒绝创建
...
- WHEN MDM-User 更新 VehicleNode 且记录不存在 THEN THE SYSTEM SHALL 返回错误码 812001（VEHICLE_NODE_NOT_EXIST）并拒绝更新
...
- WHEN MDM-User 创建 VehicleNode 且 nodeCode 与表内已存在记录冲突（无论已存在记录 status=DRAFT/ACTIVE/INACTIVE 任何状态）THEN THE SYSTEM SHALL 返回错误码 812002（VEHICLE_NODE_CODE_EXIST）并拒绝创建
...
> **决策说明**：本 US 在"反查下游引用 + 硬拒绝"与"软删除 + 下游事件自决"两种策略中，本期选择**前者（反查 + 硬拒绝）**。理由：(1) 任务方案已分配错误码 812003（VEHICLE_NODE_HAS_DOWNSTREAM_REF），与硬拒绝路径自洽；(2) 业务侧倾向于在主数据层面强阻塞，避免下游静默处理失败导致的数据漂移。是否未来切换为软删除策略由 Q13 跟踪。
...
- WHEN 反查结果存在引用 THEN THE SYSTEM SHALL 返回错误码 812003（VEHICLE_NODE_HAS_DOWNSTREAM_REF）并拒绝删除；错误响应中 SHALL 包含引用记录的统计信息（引用方服务名、引用数量、至多前 10 条引用方业务标识）
...
- WHEN MDM-User 创建 Plant 且 code 已存在 THEN THE SYSTEM SHALL 返回错误码 813002（PLANT_CODE_EXIST）并拒绝创建
...
- WHEN MDM-User 更新 Plant 且记录不存在 THEN THE SYSTEM SHALL 返回错误码 813001（PLANT_NOT_EXIST）并拒绝更新
...
- IF Plant 的 effective_from > effective_to THEN THE SYSTEM SHALL 返回错误码 813004（PLANT_EFFECTIVE_PERIOD_INVALID）并拒绝保存
...
- WHEN MDM-User 创建 Plant 且 code 与表内已存在记录冲突（无论已存在记录 status=DRAFT/ACTIVE/INACTIVE 任何状态）THEN THE SYSTEM SHALL 返回错误码 813002（PLANT_CODE_EXIST）并拒绝创建
...
- WHEN 调用历史版本查询接口且 code 不存在 THEN THE SYSTEM SHALL 返回错误码 813001（PLANT_NOT_EXIST）并拒绝查询
...
- WHEN 反查结果存在引用 THEN THE SYSTEM SHALL 返回错误码 813003（PLANT_HAS_DOWNSTREAM_REF）并拒绝删除；错误响应中 SHALL 包含引用记录的统计信息（引用方服务名、引用数量、至多前 10 条引用方业务标识）
...
- WHEN MDM-User 创建 MaterialCategory 且 code 已存在 THEN THE SYSTEM SHALL 返回错误码 814002（MATERIAL_CATEGORY_CODE_EXIST）并拒绝创建
- WHEN MDM-User 创建 MaterialCategory 且 name 为空 THEN THE SYSTEM SHALL 返回参数校验错误并拒绝创建
- WHEN MDM-User 创建 MaterialCategory 且 parentCode 指向不存在的 MaterialCategory THEN THE SYSTEM SHALL 返回错误码 814005（MATERIAL_CATEGORY_PARENT_NOT_EXIST）并拒绝创建
- WHEN MDM-User 创建 MaterialCategory 且 parentCode 形成环路（A→B→...→A）THEN THE SYSTEM SHALL 返回错误码 814006（MATERIAL_CATEGORY_LOOP_DETECTED）并拒绝创建
...
- WHEN MDM-User 更新 MaterialCategory 且记录不存在 THEN THE SYSTEM SHALL 返回错误码 814001（MATERIAL_CATEGORY_NOT_EXIST）并拒绝更新
- WHEN MDM-User 删除 MaterialCategory 且 status=DRAFT THEN THE SYSTEM SHALL 物理删除记录
- WHEN MDM-User 删除 MaterialCategory 且 status≠DRAFT THEN THE SYSTEM SHALL 返回错误码 814003 并拒绝删除
- WHEN MDM-User 删除 MaterialCategory 且存在子项 MaterialCategory（parentCode=该记录 code）THEN THE SYSTEM SHALL 返回错误码 814003（MATERIAL_CATEGORY_HAS_CHILDREN）并拒绝删除
- WHEN MDM-User 删除 MaterialCategory 且存在 Part 引用该 MaterialCategory（categoryCode=该记录 code）THEN THE SYSTEM SHALL 返回错误码 814003 并拒绝删除
...
- IF MaterialCategory 的 effective_from > effective_to THEN THE SYSTEM SHALL 返回错误码 814004（MATERIAL_CATEGORY_EFFECTIVE_PERIOD_INVALID）并拒绝保存
...
- WHEN MDM-User 删除 MaterialCategory 且存在子项 MaterialCategory（parentCode=该记录 code）THEN THE SYSTEM SHALL 返回错误码 814003 并拒绝删除，错误响应中包含子项数量信息
- WHEN MDM-User 删除 MaterialCategory 且存在 Part 引用该 MaterialCategory（categoryCode=该记录 code）THEN THE SYSTEM SHALL 返回错误码 814003 并拒绝删除，错误响应中包含引用 Part 数量信息
...
- WHEN MDM-User 更新 MaterialCategory 的 parentCode 且形成环路 THEN THE SYSTEM SHALL 返回错误码 814006 并拒绝更新
...
- WHEN MDM-User 创建 Part 且 code 已存在 THEN THE SYSTEM SHALL 返回错误码 814010（PART_CODE_EXIST）并拒绝创建
- WHEN MDM-User 创建 Part 且 name 为空 THEN THE SYSTEM SHALL 返回参数校验错误并拒绝创建
- WHEN MDM-User 创建 Part 且 categoryCode 指向不存在或 status≠ACTIVE 的 MaterialCategory THEN THE SYSTEM SHALL 返回错误码 814011（PART_CATEGORY_INVALID）并拒绝创建
- WHEN MDM-User 创建 Part 且 partType 取值不在枚举范围 THEN THE SYSTEM SHALL 返回参数校验错误并拒绝创建
- WHEN MDM-User 创建 Part 且 vehicleNodeCode 指向不存在于 EEAD.VehicleNode 的记录 THEN THE SYSTEM SHALL 返回错误码 814012（PART_VEHICLE_NODE_INVALID）并拒绝创建
- WHEN MDM-User 创建 Part 且 supplierCode 指向不存在于 Party.Supplier 的记录 THEN THE SYSTEM SHALL 返回错误码 814013（PART_SUPPLIER_INVALID）并拒绝创建
- WHEN MDM-User 创建 Part 且 substitutePartCode 指向不存在的 Part THEN THE SYSTEM SHALL 返回错误码 814014（PART_SUBSTITUTE_INVALID）并拒绝创建
...
- WHEN MDM-User 更新 Part 且记录不存在 THEN THE SYSTEM SHALL 返回错误码 814010 并拒绝更新
...
- IF Part 的 effective_from > effective_to THEN THE SYSTEM SHALL 返回错误码 814004 并拒绝保存
...
- WHEN MDM-User 创建 Part 且 categoryCode 指向不存在的 MaterialCategory THEN THE SYSTEM SHALL 返回错误码 814011 并拒绝创建
- WHEN MDM-User 创建 Part 且 categoryCode 指向 status≠ACTIVE 的 MaterialCategory THEN THE SYSTEM SHALL 返回错误码 814011 并拒绝创建
...
- WHEN MDM-User 创建 Part 且 vehicleNodeCode 不为空且指向不存在于 EEAD.VehicleNode 的记录 THEN THE SYSTEM SHALL 返回错误码 814012 并拒绝创建
...
- WHEN MDM-User 创建 Part 且 supplierCode 不为空且指向不存在于 Party.Supplier 的记录 THEN THE SYSTEM SHALL 返回错误码 814013 并拒绝创建
...
- WHEN 调用历史版本查询接口且 code 不存在 THEN THE SYSTEM SHALL 返回错误码 814001 或 814010 并拒绝查询
...
- WHEN MDM-User 更新 Part 的 lifecycleStage 且新状态为逆向跳转（如 MASS_PRODUCTION → PROTOTYPE）THEN THE SYSTEM SHALL 返回错误码 814015（PART_LIFECYCLE_INVALID_TRANSITION）并拒绝更新
- WHEN MDM-User 更新 Part 的 lifecycleStage 且当前状态为 OBSOLETE THEN THE SYSTEM SHALL 返回错误码 814015 并拒绝更新（OBSOLETE 为终态，禁止任何变更）
...
- WHEN MDM-User 创建或更新 Part 且 substitutePartCode 指向不存在的 Part THEN THE SYSTEM SHALL 返回错误码 814014（PART_SUBSTITUTE_INVALID）并拒绝操作
- WHEN MDM-User 创建或更新 Part 且 substitutePartCode 指向自身（code=substitutePartCode）THEN THE SYSTEM SHALL 返回错误码 814014 并拒绝操作
...
- WHEN MDM-User 创建 Part 且 code 与表内已存在记录冲突 THEN THE SYSTEM SHALL 返回错误码 814010（PART_CODE_EXIST）并拒绝创建
- WHEN MDM-User 创建 MaterialCategory 且 code 与表内已存在记录冲突 THEN THE SYSTEM SHALL 返回错误码 814002（MATERIAL_CATEGORY_CODE_EXIST）并拒绝创建
- THE SYSTEM SHALL 在错误响应体中明确指出冲突的 code 取值与已占用记录的当前 status，便于排查
...
- WHEN 检查结果存在引用 THEN THE SYSTEM SHALL 返回错误码 814016（PART_HAS_DOWNSTREAM_REF）并拒绝删除；错误响应中包含引用记录数量信息
...
- **VehicleNode 删除策略**：本期 CR-007 采用"反查下游引用 + 硬拒绝"策略（详见 US-045 决策说明）；DRAFT 状态记录可直接物理删除，ACTIVE / INACTIVE 状态须先反查下游 source=MDM 的本地副本引用，存在引用则返回 812003 拒绝；MDM-Admin 通过专用权限点 `mdm:eead:vehicleNode:remove:force` 提供旁路能力但须留审计
...
- **Plant 删除策略**：沿用 US-045（VehicleNode）的"反查下游引用 + 硬拒绝"策略（详见 US-054 决策说明）；DRAFT 状态记录可直接物理删除，ACTIVE / INACTIVE 状态须先反查下游 source=MDM 的本地副本引用，存在引用则返回 813003 拒绝；MDM-Admin 通过专用权限点 `mdm:org:plant:remove:force` 提供旁路能力但须留审计
...
- **Part 删除策略**：采用"反查下游引用 + 硬拒绝"策略（沿用 VehicleNode / Plant 同款）；DRAFT 状态记录可直接物理删除，ACTIVE / INACTIVE 状态须先反查其他 Part 的 substitutePartCode 引用，存在引用则返回 814016 拒绝；MDM-Admin 通过专用权限点 `mdm:material:part:remove:force` 提供旁路能力但须留审计
...
- **Org 子域错误码段位**：Org 子域分配 **813xxx 段**（与 EEAD 的 812xxx、Product/Party 的 807xxx 错开）：813001（PLANT_NOT_EXIST）/ 813002（PLANT_CODE_EXIST）/ 813003（PLANT_HAS_DOWNSTREAM_REF）/ 813004（PLANT_EFFECTIVE_PERIOD_INVALID）/ 813010 ~ 813013（预留给未来上游接入路径）
...
| 编号 | 问题 | 影响范围 | 建议（待业务确认） |
|------|------|----------|-------------------|
| Q1 | 上层失效（如 Model → INACTIVE）时，是否级联失效下层（Variant / Configuration）？还是仅校验阻止？ | US-019 / US-020 / US-022 | **已决议：采用方案 B（阻止失效 + 依赖检查）**。失效父级时检查是否存在 ACTIVE 状态的子级，存在则拒绝失效并返回错误码（807016~807022）。详见 Changelog CR-010 |
...
| Q12 | 错误码段位现状不统一：MDM 服务规范段位为 812xxx，但 Product MDM / Party MDM 历史 CR（CR-001 ~ CR-006）已使用 807xxx 段（含 807001 ~ 807015 / 807020 共 12 个错误码），EEAD 子域本期使用 812xxx 段（812001 ~ 812003）。是否需要立独立 CR 把现有 807xxx 错误码统一迁移至 812xxx 段下重新分配？ | §5 / 全部 US 错误码引用 / design §5.5 错误码表 | 建议立独立 CR 处理：本期保持 EEAD 用 812xxx 段，Product / Party 沿用 807xxx 段；未来由"错误码段位统一对齐 CR"集中迁移并同步更新 framework 错误码注册表（待 MDM-Admin / 架构师确认） |
| Q13 | VehicleNode 删除策略本期采用"反查下游引用 + 硬拒绝"（US-045 方案 A），未来是否需要切换为"软删除标记 + 下游事件自决"策略？ | US-045 | 建议运行一段时间后基于实际业务反馈复盘：若反查 VMD 的 Feign 调用频繁失败导致删除接口可用性下降，或下游本地副本清理流程已成熟到可信任 by event，再立 CR 切换为软删除策略（待业务与架构师确认） |
| Q14 | VehicleNode 是否需要开启上游接入路径（订阅 `upstream.<sourceSystem>.eead.vehicleNode` topic 与暴露 `POST /api/upstream/mdm/eead/vehicleNode/v1/ingest` 接口）？本期默认仅 LOCAL 维护，source 字段保留 MDM 取值供未来扩展使用 | US-046 / OS-17 | 建议本期不开启；若未来 PLM / 集团主数据平台 / OEM 工程定义系统能作为权威源推送 VehicleNode 主数据，再立独立 CR 开启接入路径并复用 US-013 ~ US-018 已建立的接入处理链路（待 E/E 架构工程师确认） |
| Q15 | Plant 的 `code` 是否需要符合预设命名规范（如 `PLT_<国家>_<城市>_<序号>`）？是否在系统层做正则校验，还是仅在用户文档约定？ | US-047 | 建议首期仅在用户文档约定命名规范，不做系统层正则校验；待业务确认 |
| Q16 | `legalEntityCode` / `costCenterCode` 本期作为字符串字段不做引用完整性校验，未来 Legal Entity 子域落地时是否回填外键约束 + 数据回扫？ | US-047 / §5 约束 | 建议未来 Legal Entity / Cost Center 落地时回填外键约束并执行数据回扫脚本（待架构师确认） |
| Q17 | Plant 失效（INACTIVE）是否需要进一步校验下层引用？参考 Supplier（Q11 决议为不校验，仅发事件）vs VehicleNode（US-045 反查 + 硬拒绝）两种范式选择 | US-054 | 建议沿用 VehicleNode 的反查策略（反查 + 硬拒绝），因为 Manufacturer / Plant 在 VMD 中是 `Vehicle` 的强引用字段，悬空风险更高（待业务确认） |
| Q18 | 未来 ERP 上线后，Plant 的权威源切换（LOCAL → ERP）流程是否需要独立的灰度方案？ | US-055 / OS-23 / §5 约束 | 建议 ERP 上线时立独立 CR 设计权威源切换灰度方案（含双写期、数据校验期、切换期三阶段），参考 US-017 权威源配置机制扩展（待 ERP 项目计划确认） |
| Q19 | Manufacturer（制造商，语义更宽，可对应多个 Plant）与 Plant 是否需要拆为两个独立实体？ | US-047 / §1 Overview | 建议本期合并语义到 Plant（一条 Plant 记录即代表一个独立物理工厂，由 legalEntityCode 间接表达制造商归属）；如未来出现"一家制造商有多个 Plant 需要在 MDM 内显式建模"的诉求，再立独立 CR 拆分（待业务确认） |
| Q20 | Part 的 partType 枚举是否需要更细化的分类？如区分原材料 / 标准件 / 定制件 / 软件件 / 总成件 / 电子件 / 机械件等 | US-059 | 建议首期采用 RAW_MATERIAL / STANDARD_PART / CUSTOM_PART / SOFTWARE / ASSEMBLY 五类，未来按业务反馈扩展（待物料工程确认） |
| Q21 | Part 的 vehicleNodeCode 是否允许引用 status≠ACTIVE 的 VehicleNode？ | US-061 | 建议仅允许引用 ACTIVE 状态的 VehicleNode，与 MaterialCategory 引用策略一致（待业务确认） |
| Q22 | MaterialCategory 的树形层级深度是否需要限制？ | US-058 | 建议首期不做系统层深度限制，在用户文档约定合理深度（如不超过 5 层）；待业务确认 |
| Q23 | Part 的 substitutePartCode 是否需要支持一对多替代关系（一个零件有多个可选替代件）？ | US-068 | 建议首期仅支持一对一替代（一个 substitutePartCode 字段），一对多替代关系留待 OS-26 SubstituteRelation 独立实体管理（待业务确认） |
| Q24 | Part 的 fotaUpgradeable 字段与 VehicleNode 的 otaSupportType 字段如何协同？当物料级声明 fotaUpgradeable=true 但节点级声明 otaSupportType=NOT_SUPPORTED 时如何处理？ | US-059 | 建议两个字段独立管理，不做强制一致性校验；由下游 OTA 服务自行决策（待 OTA 服务确认） |
| Q25 | Material 子域的错误码段位是否沿用 814xxx 段？还是需要与其他子域对齐？ | §5 | 建议采用 814xxx 段（与 EEAD 812xxx / Org 813xxx 错开），待架构师确认 |
