...
│   │   │   ├── ConfigurationCreateCmd.java
│   │   │   ├── ConfigurationUpdateCmd.java
...
|------|------|------|------|
| id | BIGINT | Y | 主键，自增 |
| aggregate_type | VARCHAR(32) | Y | 聚合类型（BRAND / SERIES / PLATFORM / MODEL / VARIANT / CONFIGURATION / OPTION_FAMILY / OPTION_CODE） |
...
|------|------|------|------|
| id | BIGINT | Y | 主键，自增 |
| entity_type | VARCHAR(16) | Y | 实体类型（BRAND / SERIES / PLATFORM / MODEL / VARIANT / CONFIGURATION / OPTION_FAMILY / OPTION_CODE） |
...
| Method | Path | 说明 |
|--------|------|------|
| POST | /api/mpt/mdm/configuration/v1/create | 创建配置 |
| PUT | /api/mpt/mdm/configuration/v1/{code} | 更新配置 |
| DELETE | /api/mpt/mdm/configuration/v1/{code} | 删除配置（仅 DRAFT 状态） |
...
| Method | Path | 说明 |
|--------|------|------|
| POST | /api/upstream/mdm/configuration/v1/ingest | 接收上游 Configuration 主数据 |
...
| US-020 | §3.1 (mdm_variant), §4 (F1 同模式), §5.1 (Variant API) | Variant CRUD + 引用校验 + 组合查询 |
| US-021 | §3.1 (mdm_variant_option_code_binding), §4 (F7), §5.1 (Variant 绑定 API) | Variant 绑定/解绑 Option Code + 互斥校验 |
| US-022 | §3.1 (mdm_configuration), §4 (F1 同模式), §5.1 (Configuration API) | Configuration CRUD + 引用校验 |
...
| US-030 | §4 (F5/F6), §5.3 (5 类新实体 Upstream API) | 5 类新实体上游接入扩展 |
